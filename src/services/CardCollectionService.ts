...
export class CardCollectionService {
   ...
   * @throws Error if card collection not found or update fails
   */
  async updatePaymentId(id: string, paymentId: string): Promise<CardCollection> {
    const query = `
      UPDATE card_collections
    ...
    try {
      const result = await pool.query<CardCollectionRow>(query, [paymentId, id]);
      ...
      throw new Error(`Failed to update payment ID: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   * @returns Card collection if found, null otherwise
   */
  async findByPaymentId(paymentId: string): Promise<CardCollection | null> {
    const query = `
      SELECT * FROM card_collections
      WHERE payment_id = $1
    `;
    ...
    try {
      const result = await pool.query<CardCollectionRow>(query, [paymentId]);
      ...
      console.error('Error finding card collection by payment ID:', error);
      throw new Error(`Failed to find card collection: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
