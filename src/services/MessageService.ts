...
export class MessageService {
   ...
   * @throws Error if message not found or update fails
   */
  async updatePaymentId(id: string, paymentId: string): Promise<Message> {
    const query = `
      UPDATE messages
    ...
    try {
      const result = await pool.query<MessageRow>(query, [paymentId, id]);
   ...
   * @returns Message if found, null otherwise
   */
  async findByPaymentId(paymentId: string): Promise<Message | null> {
    const query = `
      SELECT * FROM messages
      WHERE payment_id = $1
    `;
    ...
    try {
      const result = await pool.query<MessageRow>(query, [paymentId]);
      ...
      console.error('Error finding message by payment ID:', error);
      throw new Error(`Failed to find message: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
