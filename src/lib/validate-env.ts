...
async function main() {
  ...
  try {
    ...
    // Check if using test mode in development (MercadoPago)
    const config = validateEnv();
    if (config.NODE_ENV === 'development' && !config.MERCADOPAGO_ACCESS_TOKEN.startsWith('TEST-')) {
      console.warn('⚠ WARNING: Using LIVE MercadoPago keys in development mode!');
      console.warn('  Consider using TEST keys (TEST-...) for development.\n');
    }
    
    // Check if using test mode in production
    if (config.NODE_ENV === 'production' && config.MERCADOPAGO_ACCESS_TOKEN.startsWith('TEST-')) {
      console.error('✗ ERROR: Using TEST MercadoPago keys in production mode!');
      console.error('  You must use LIVE keys for production.\n');
      process.exit(1);
    }
