...
export function CardModal({ card, isFirstOpen, onClose, coverImageUrl = null }: CardModalProps) {
  ...
  useEffect(() => {
    if (isFirstOpen) {
      ...
      const emojiTimer = setTimeout(() => setShowEmojis(false), 8000);
      return () => { clearTimeout(contentTimer); clearTimeout(emojiTimer); };
    } else {
      setShowContent(true);
    }
  }, [isFirstOpen]);
  ...
  return (
    <div
      ...
      onClick={handleClose}
    >
      {showEmojis && <FallingEmojis emoji={getEmojiFromTitle()} count={20} />}

      {/* Close X — fixed top-right */}
      <button
        onClick={handleClose}
      ...
      <div
        className={`
          relative bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto
      ...
      >
        ...
        <div className="bg-gradient-to-br from-blue-500 to-purple-600 text-white p-8 rounded-t-3xl">
          ...
          <h2 id="card-modal-title" className="text-3xl font-bold text-center mb-2">
            Carta {card.order}
          </h2>
        ...
        <div className={`p-8 space-y-6 ${isFirstOpen && !showContent ? 'opacity-0' : 'opacity-100 transition-opacity duration-1000'}`}>
          ...
          <div className="bg-blue-50 border-2 border-blue-200 rounded-2xl p-6">
            <p className="text-lg text-gray-800 leading-relaxed whitespace-pre-wrap">{card.messageText}</p>
          </div>
          ...
          {card.youtubeUrl && (
            <div className="space-y-3">
              ...
              <div className="rounded-2xl overflow-hidden shadow-lg aspect-video">
                <YouTubePlayer videoUrl={card.youtubeUrl} autoplay={isFirstOpen} volume={70} className="w-full h-full" />
              </div>
            </div>
          )}
          ...
          {isFirstOpen && showContent && (
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-200 rounded-2xl p-6 text-center animate-in fade-in duration-1000 delay-500">
              <p className="text-amber-900 font-medium">
                ✨ Esta é a primeira e única vez que você pode ver esta carta completa. Aproveite este momento especial! ✨
              </p>
            </div>
          )}
          ...
          <div className="flex justify-center pt-4">
            <Button variant="primary" onClick={handleClose} className="px-8 py-3 text-lg" aria-label="Fechar carta">
              Fechar Carta
            </Button>
          </div>
        </div>
      ...
      {/* Confirmation dialog — only on first open */}
      {showCloseConfirm && (
        <div
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 space-y-5 animate-in fade-in zoom-in duration-200">
            <div className="text-center space-y-3">
              ...
              <h3 className="text-xl font-bold text-gray-900">Fechar esta carta?</h3>
              <p className="text-sm text-gray-600 leading-relaxed">
                Ao fechar, esta carta será selada para sempre. Você não poderá abri-la novamente.
              </p>
