...
def cmd_image(args):
    ...
    Aceita --url (URL publica) ou --file (arquivo local).
    Se --file for usado, a imagem e automaticamente enviada para o R2 da Paperbloom
    (meta-ads/criativos/<produto>/) e a URL publica e usada para o upload na Meta.
    """
    init_api()
    import requests
    import tempfile
    ...
    # Resolver URL: --file faz upload no R2 primeiro, --url usa direto
    if args.file:
        if not args.url:
            # Upload para R2 e obter URL publica
            sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            try:
                from r2 import upload_file
            except ImportError:
                print_error("r2.py nao encontrado. Verifique a instalacao da skill.")
                sys.exit(1)
            produto = args.produto or 'geral'
            print(f"Enviando para R2 (meta-ads/criativos/{produto}/)...", file=sys.stderr)
            url = upload_file(args.file, produto=produto)
        else:
            print_error("Use --file OU --url, nao ambos.")
            sys.exit(1)
    elif args.url:
        url = args.url
    else:
        print_error("Informe --file (arquivo local) ou --url (URL publica).")
        sys.exit(1)

    # Download da URL para arquivo temporario (Meta exige upload de arquivo)
    print(f"Enviando imagem para Meta Ads...", file=sys.stderr)
    response = requests.get(url, stream=True)
    response.raise_for_status()

    # Determinar extensao pelo content-type ou URL
    ext = '.jpg'
    ...
    try:
        ...
        # Incluir a URL do R2 no resultado se foi upload local
        if args.file:
            result['r2_url'] = url
...
def main():
    ...
    # --- image ---
    p = sub.add_parser("image", help="Upload de imagem (local via R2 ou URL publica)")
    add_account_arg(p)
    p.add_argument("--url", help="URL publica da imagem (alternativa ao --file)")
    p.add_argument("--file", help="Caminho do arquivo local (faz upload no R2 automaticamente)")
    p.add_argument("--produto", choices=['mensagem-digital', '12-cartas', 'revelacao-virtual', 'geral'],
                   default='geral', help="Produto para organizar no R2 (default: geral)")
    p.add_argument("--name", help="Nome da imagem no Meta Ads")
