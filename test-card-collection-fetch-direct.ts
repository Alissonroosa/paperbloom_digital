/**
 * Direct test for Card Collection Fetch API Routes
 * Tests the route handlers directly without needing a running server
 * 
...
import { GET as getById } from './src/app/api/card-collections/[id]/route';
import { GET as getBySlug } from './src/app/api/card-collections/slug/[slug]/route';
...
async function runTests() {
  console.log('🧪 Testing Card Collection Fetch API Routes (Direct)\n');
  ...
  try {
    ...
    const uniqueSlug = `test-recipient-12-cartas-${timestamp}`;
    ...
    try {
      const request = new NextRequest(`http://localhost:3000/api/card-collections/${testCollectionId}`);
      const response = await getById(request, { params: { id: testCollectionId } });
    ...
    try {
      const request = new NextRequest(`http://localhost:3000/api/card-collections/slug/${testSlug}`);
      const response = await getBySlug(request, { params: { slug: testSlug } });
      const data = await response.json();
    ...
    console.log('\n--- Test 3: Invalid ID Format ---');
    try {
      const request = new NextRequest('http://localhost:3000/api/card-collections/invalid-id');
    ...
    try {
      ...
      const request = new NextRequest(`http://localhost:3000/api/card-collections/${fakeId}`);
      const response = await getById(request, { params: { id: fakeId } });
      const data = await response.json();
    ...
    console.log('\n--- Test 5: Non-existent Slug ---');
    try {
      const request = new NextRequest('http://localhost:3000/api/card-collections/slug/non-existent-slug');
      const response = await getBySlug(request, { params: { slug: 'non-existent-slug' } });
      const data = await response.json();
    ...
    console.log('\n--- Test 6: Empty Slug ---');
    try {
      const request = new NextRequest('http://localhost:3000/api/card-collections/slug/');
      const response = await getBySlug(request, { params: { slug: '' } });
      const data = await response.json();
      ...
      if (response.status === 400 && data.error?.code === 'INVALID_SLUG') {
        logResult(
          'Empty Slug',
          true,
          'Returns 400 with INVALID_SLUG error code'
        );
      } else {
        logResult(
          'Empty Slug',
          false,
          `Expected 400 with INVALID_SLUG, got ${response.status}`
        );
      }
    } catch (error) {
      logResult(
